...
class MedicineTypeVC: UIViewController {
    ...
    @objc func tappedNextBtn() {
        presenter?.storeValueWithPush(medName: objMedicine?.medName ?? "", medType: selectedIndexType, firstDose: "", hr: 0, min: 0, sec: 0, isUpdate: objMedicine?.isEdit ?? false, index: objMedicine?.index ?? 0, medicineLbl: medicineNameLbl, navigationController: navigationController!)
    }
...
extension MedicineTypeVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        presenter?.showCellForRow(tableView: tableView, arrMedicine: arrMedicine, selectedIndex: selectedIndex, indexPath: indexPath, nextBtn: nextBtn) ?? UITableViewCell()
    }
